      ...
      <div className="relative z-10 flex-1 flex flex-col justify-center px-4 sm:px-6 lg:px-8 max-w-[1200px] mx-auto w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-5xl text-black mb-5 sm:mb-6 leading-tight">
              ...
              <br />
              <span className="text-[#625FD0]">Before Problems Surface.</span>
            </h1>

            <p className="text-sm sm:text-md text-gray-600 leading-relaxed mb-8 sm:mb-10 max-w-lg">
              SuperManager AGI’s Project Intelligence layer gives every leader a
              complete, live view of execution progress across every active
              initiative retrieved directly from connected systems in real time
              via the ADA layer. Instantly surface risks, understand what is
              actually happening and receive grounded intelligence to keep every
            ...
            <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mt-10 sm:mt-12">
              <StatCard
                value="92.5%"
                label="Retrieval accuracy via HNSW vector search and per-subtask RAG"
                delay={200}
              />
              <StatCard
                value="3×"
                label="Faster risk detection vs manual monitoring cycles"
                delay={350}
              />
              <StatCard
                value="60ms"
                label="Data retrieval latency via ADA direct database access"
                delay={500}
              />
              <StatCard
                value="81.3%"
                label=" 22.4% to 4.2% vs baseline"
                delay={200}
              />
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 mt-14 sm:mt-20">
          <FeatureCard
            ...
            body="Track progress across every sprint and milestone in real time. Instantly see where projects stand and what needs attention."
            delay={200}
          />
          <FeatureCard
            icon="🧠"
            title="Predictive Risk Detection"
            body="AI continuously analyzes timelines, workloads, and dependencies to surface risks before delays or blockers impact delivery."
            delay={350}
          />
          <FeatureCard
            icon="🎯"
            title="AI Project Co-Pilot"
            body="Managers receive intelligent recommendations, automated status updates, and guidance to keep teams aligned and productive."
            delay={500}
          />
        </div>
      </div>
